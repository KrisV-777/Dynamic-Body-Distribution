...
option("install_path")
    ...
    set_description("Set default installation path for commands and auto_install",
                    'Defaults to: "%XSE_TES5_MODS_PATH%\\Dynamic-Body-Distribution"')
    on_check(function(option)
        import("dotenv.check")(option, {"$(env XSE_TES5_MODS_PATH)", "Dynamic-Body-Distribution"})
    end)
option_end()
...
target(PROJECT_NAME)
    ...
    add_rules("commonlibsse-ng.plugin", {
        name = PROJECT_NAME,
        author = "KrisV-777",
        description = "SKSE Framework to hot swap body sliders and textures",
    })
...
target("detours")
    ...
    remove_files("lib/detours/src/uimports.cpp") -- This file is included and not compiled on its own
...
target("papyrus")
    ...
    set_basename("DynamicBodyDistribution")
...
xpack("dynamic-body-distribution")
    set_formats("zip")
    set_extension(".7z")
    set_basename("Dynamic Body Distribution - V$(version)")
